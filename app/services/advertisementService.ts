...
export async function fetchAdvertisements(): Promise<Advertisement[]> {
  try {
    ...
    return Promise.all(ads.map(async (ad, index) => {
...
export async function getUnsplashImageUrl(width: number, height: number): Promise<string> {
  try {
    const response = await fetch(`/api/unsplash?width=${width}&height=${height}`);
