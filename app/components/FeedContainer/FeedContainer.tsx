...
import type { FeedItem, Advertisement as AdvertisementType } from '../../models/types';
...
export function FeedContainer() {
  ...
  useEffect(() => {
    if (paginatedItems.length > 0) {
      ...
      const ads = paginatedItems.filter(item => item.type === 'advertisement') as AdvertisementType[];
      const items = paginatedItems.filter(item => item.type !== 'advertisement');
      ...
      setError('Không có dữ liệu để hiển thị. Vui lòng kiểm tra kết nối mạng và thử lại.');
    }
  ...
  useEffect(() => {
    const resizeGridItems = () => {
      ...
      try {
        ...
        const rowGapStr = gridStyle.getPropertyValue('grid-gap') || gridStyle.getPropertyValue('gap');
  ...
  return (
    <div className={styles.container} ref={containerRef}>
      ...
      <div className={styles.pagination}>
        ...
        >
          Trang tiếp
          <span className={styles.buttonIcon} style={{ marginLeft: '8px', marginRight: 0 }}>→</span>
        </button>
      </div>
    </div>
  );
